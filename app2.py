...
# ================= 2. 页面基础渲染 & ASIN 档案中枢 =================
st.set_page_config(page_title="全景备货推演沙盘 V7.4", layout="wide")
st.title("📦 亚马逊【全链路物流】精算沙盘 (V7.4 全局统控版)")
st.markdown("已升级：双向时光机 | 一键全局建单锁定 | 发车动态追踪雷达 | 面板纯净折叠")
...
box_qty = col_b3.number_input("📦 单箱数", min_value=1, value=int(c_data.get("box_qty", 24)), key=f"box_qty_{asin_name}")
...
saved_transit_list = c_data.get("transit_list", [{"ch": "HQYD-海运快线-限时达", "ship_date": start_date.strftime('%Y-%m-%d'), "qty": 0}] * 5)
...
for i in range(st.session_state[t_count_key]):
    ...
    with st.sidebar.expander(f"在途货件 {i+1}", expanded=(i==0 or t_qty_saved > 0)):
        ...
        if t_qty > 0:
            ...
            if days_to_arr > 0: baseline_batches.append({"name": f"在途:{t_ch.split('-')[-1]}", "qty": t_qty, "day": days_to_arr})
...
saved_prod = c_data.get("prod", [{"ch": "HQYD-海运快线-限时达", "qty": 0}] * 3)
p_count_key = f"p_count_{asin_name}"
if p_count_key not in st.session_state: st.session_state[p_count_key] = max(3, len(saved_prod))
...
for i in range(st.session_state[p_count_key]):
    ...
    if b_key not in st.session_state:
        st.session_state[b_key] = int(saved_p.get("buffer", 4))
    ...
    with st.sidebar.expander(f"生产批次 {i+1} (手工预排参考)", expanded=False):
        ...
        p_ship = c_2.date_input("🚢实际开船/起飞日", key=s_key, on_change=sync_bwd)
...
for i in range(st.session_state[n_count_key]):
    ...
    with st.sidebar.expander(f"发货分仓 {i+1}", expanded=(i==0)):
        ...
        if new_qty > 0:
            ...
            user_new_batches.append({"name": f"新单:{new_ch.split('-')[-1]}", "qty": new_qty, "day": total_days_needed})
...
# ================= 6. AI 大脑：双擎探测中枢 =================
with st.expander("🤖 V7.4 AI 资金杠杆调控与核验中枢 (点击展开查阅详细策略与矩阵)", expanded=False):
    ...
    def get_first_drop_days(current_batches):
        ...
        for d in range(1, 365):
            ...
            temp_stock -= daily_sales_array[d - 1]
    ...
    def get_final_drop_days(current_batches):
        t_15, t_0 = None, None
        temp_stock_arr, safe_level_arr = [], []
        temp_stock = initial_stock
        for d in range(1, 366):
            ...
            temp_stock -= daily_sales_array[d - 1]
            safe_level = sum(daily_sales_array[d : d + 15])
            temp_stock_arr.append(temp_stock)
            safe_level_arr.append(safe_level)
            
        last_healthy_15 = 0
        for d in range(365, 0, -1):
            if temp_stock_arr[d-1] >= safe_level_arr[d-1]:
                last_healthy_15 = d
                break
        t_15 = last_healthy_15 + 1 if last_healthy_15 < 365 else 365
        
        last_healthy_0 = 0
        for d in range(365, 0, -1):
            if temp_stock_arr[d-1] >= 0:
                last_healthy_0 = d
                break
        t_0 = last_healthy_0 + 1 if last_healthy_0 < 365 else 365
        return t_15, t_0
    ...
    base_t_15, base_t_0 = get_first_drop_days(baseline_batches)
    ...
    if base_t_15 >= 360:
        ...
        ai_prod_batches = [{"name": f"生产中(手工排期)", "qty": b["qty"], "day": b["deliv_offset"] + LOGISTICS_CHANNELS[b["manual_ch"]] + fba_delay} for b in prod_batches_raw]
        ...
        test_t_15, test_t_0 = get_final_drop_days(test_batches)
        new_need_qty = max(0, sum(daily_sales_array[test_t_15 : test_t_15 + cover_days]))
        ...
        for pb in prod_batches_sorted:
            cur_t_15, cur_t_0 = get_first_drop_days(current_sim_batches) 
            ...
            arr_day = pb["deliv_offset"] + LOGISTICS_CHANNELS[best_ch] + fba_delay
            sim_batch = {"name": f"采购中:{best_ch.split('-')[-1]}", "qty": final_qty, "day": arr_day, "hide_label": False}
            ai_prod_batches.append(sim_batch)
            current_sim_batches.append(sim_batch)
            
            status = "🟢 完美压线" if reason in ["P1", "P2"] else ("🟡 容忍降速" if "P3" in reason else "🚨 极速空运")
            ai_prod_reports.append(f"* **批次 {pb['id']+1}**：产出 `{final_qty}件 ({final_boxes}箱)`{fee_warn} $\\rightarrow$ **系统推荐发往 [{best_ch}]**。({status})")

        final_sim_batches = current_sim_batches.copy()
        final_t_15, final_t_0 = get_final_drop_days(current_sim_batches)
        demand_cover_qty = sum(daily_sales_array[final_t_15 : final_t_15 + cover_days])
    ...
    if base_t_15 < 360:
        final_date_str = (start_date + datetime.timedelta(days=final_t_15)).strftime('%Y-%m-%d')
        ...
        deadline_offset = final_t_15 - total_fastest_time
        
        if deadline_offset > 0:
            deadline_date = start_date + datetime.timedelta(days=deadline_offset)
            st.write(f"（理论释义：系统叠加计算老订单后，大盘终极安全断崖延至 {final_t_15} 天后 ({final_date_str})。按系统配置的最快海运（{fastest_sea_name} {fastest_sea_time}天 + 备货交期 {global_lead}天 + 上架 {fba_delay}天 = 共 {total_fastest_time}天）反推，**您的极限安全建单节点为 {deadline_offset} 天后 ({deadline_date.strftime('%Y-%m-%d')})**。系统基于此断崖点为您动态推算出以下理论补货极值：）")
            
            st.success(f"✅ **【安全储备期：无需采购】** 经系统反推，您的资金目前极其充裕！建议您在 **{deadline_offset} 天后 ({deadline_date.strftime('%Y-%m-%d')})** 再来处理本批次采购计划。")
            st.write(f"💡 **未来采购策略预演**：假设您在 {deadline_date.strftime('%Y-%m-%d')} 当天准时建单，系统动态推演的推荐采购量及分仓策略如下：")
            
            simulated_lead = global_lead + deadline_offset
            display_moq_and_matrix(ai_new_qty, ai_new_boxes, user_new_total_qty, math.ceil(user_new_total_qty/box_qty) if user_new_total_qty>0 else 0, final_t_15, final_t_0, region_code, simulated_lead, effective_fast_pct)
            ...
            st.write(f"（理论释义：系统叠加计算老订单后，大盘终极安全断崖延至 {final_t_15} 天后 ({final_date_str})。按系统配置的最快海运（{fastest_sea_name} {fastest_sea_time}天 + 备货交期 {global_lead}天 + 上架 {fba_delay}天 = 共 {total_fastest_time}天）反推，**您的极限安全建单节点已透支 {abs(deadline_offset)} 天！** 系统基于此断崖点为您动态推算出以下理论补货极值：）")
            ...
            if ai_new_qty > 0:
                ...
                if user_new_total_qty == 0:
                    ...
                    if ai_new_qty < MOQ_UNITS:
                        ...
                        st.markdown(f"#### 方案A：独立成团满足 MOQ `{moq_qty}件 ({moq_boxes}箱)`")
                        lines_a, plan_a = calc_strict_matrix(moq_boxes, effective_fast_pct, final_t_15, final_t_0, region_code, global_lead)
                        for l in lines_a: st.write(l)
                        
                        st.markdown(f"#### 方案B：跨同事/店铺精益拼单 `{ai_new_qty}件 ({ai_new_boxes}箱)`")
                        st.caption(f"*(如果您想极致释放现金流，且能找到拼单，此为保底矩阵)*")
                        lines_b, plan_b = calc_strict_matrix(ai_new_boxes, effective_fast_pct, final_t_15, final_t_0, region_code, global_lead)
                        for l in lines_b: st.write(l)
                        ...
                        lines, plan = calc_strict_matrix(ai_new_boxes, effective_fast_pct, final_t_15, final_t_0, region_code, global_lead)
                    ...
                    if is_moq_intent:
                        ...
                        day_start = min([b["day"] for b in user_new_batches]) if user_new_batches else final_t_15
                        _, day_end_0 = get_final_drop_days(temp_all_batches)
                        ...
                        lines_user, plan_user = calc_strict_matrix(user_new_boxes, effective_fast_pct, final_t_15, final_t_0, region_code, global_lead)
                        for l in lines_user: st.write(l)
                        
                        st.markdown(f"#### 方案B：跨店铺拼单退路 (极限省钱 `{ai_str}`)")
                        st.caption("*(如果您想极致释放现金流，且能找到拼单，此为保底拼单矩阵)*")
                        lines_ai, plan_ai = calc_strict_matrix(ai_new_boxes, effective_fast_pct, final_t_15, final_t_0, region_code, global_lead)
        ...
        next_t_15, _ = get_final_drop_days(final_sim_batches)
        if next_t_15 >= 360:
...
for d in range(1, 366):
    ...
    for b in arr: temp_stock += b["qty"]
    sales_today = daily_sales_array[d - 1] if d <= len(daily_sales_array) else global_sales
...
hit_zero_date, hit_safety_date = None, None

for day in range(1, simulation_days + 1):
    current_date = start_date + datetime.timedelta(days=day)
    arrived_today = [b for b in final_all_batches if b["day"] == day]
    for b in arrived_today: current_stock += b["qty"] 
    sales_today = daily_sales_array[day - 1] if day <= len(daily_sales_array) else global_sales
    current_stock -= sales_today
    
    safety_stock = 0
    for sd in range(day, day + 15):
        safety_stock += daily_sales_array[sd - 1] if sd <= len(daily_sales_array) else global_sales
    ...
    if current_stock < safety_stock and hit_safety_date is None: hit_safety_date = current_date
    if current_stock < 0 and hit_zero_date is None: hit_zero_date = current_date

df_plot = pd.DataFrame(inventory_list)

c1, c2, c3, c4 = st.columns(4)
c1.metric("📦 预计未来总流水量", f"{sum(b.get('qty', 0) for b in final_all_batches)} 件")
if hit_zero_date: c2.metric("🚨 彻底断货预警", hit_zero_date.strftime("%Y-%m-%d"), "跌破 0 库存", delta_color="inverse")
else: c2.metric("✅ 彻底断货预警", "未断货", "安全", delta_color="normal")
if hit_safety_date: c3.metric("⚠️ 安全水位跌破", hit_safety_date.strftime("%Y-%m-%d"), "跌穿 15天线", delta_color="inverse")
else: c3.metric("✅ 安全水位预警", "健康", "高于安全线", delta_color="normal")
...
if hit_safety_date:
    ...
    fig.add_annotation(x=hit_safety_date.strftime('%Y-%m-%d'), y=y_val, text=f"⚠️ {hit_safety_date.strftime('%m/%d')} 跌破安全线", showarrow=True, arrowhead=1, arrowcolor="#e67700", ax=0, ay=-40, bgcolor="#fff4e6", font=dict(color="#e67700"))
if hit_zero_date:
    fig.add_annotation(x=hit_zero_date.strftime('%Y-%m-%d'), y=0, text=f"🚨 {hit_zero_date.strftime('%m/%d')} 彻底断货!", showarrow=True, arrowhead=1, arrowcolor="red", ax=0, ay=-40, bgcolor="#ffe3e3", font=dict(color="red"))

colors = ["#f8f9fa", "#e9ecef", "#dee2e6", "#ced4da"]
for i, p in enumerate(phases):
    t_start, t_end = p["start"].strftime('%Y-%m-%d'), (p["end"] + datetime.timedelta(days=1)).strftime('%Y-%m-%d')
    fig.add_vrect(x0=t_start, x1=t_end, fillcolor=colors[i % len(colors)], opacity=0.5, layer="below", line_width=0, annotation_text=p["name"], annotation_position="top left")

arrival_dict = {}
for b in final_all_batches:
    if b.get("hide_label"): continue 
    arr_date = start_date + datetime.timedelta(days=b["day"])
    if arr_date in arrival_dict: arrival_dict[arr_date] += f"<br>+ {b['name']}({b['qty']}件)"
    else: arrival_dict[arr_date] = f"📦 {b['name']}({b['qty']}件)"
...
for d_date, text in arrival_dict.items():
    ...
    try:
        ...
        fig.add_annotation(x=date_str, y=y_val, text=f"🟢 {d_date.strftime('%m/%d')} 上架<br>{text}", showarrow=True, arrowhead=1, arrowcolor="#2b8a3e", arrowsize=1.5, ax=0, ay=50, bgcolor="#ebfbee", bordercolor="#2b8a3e", font=dict(color="#2b8a3e"))
        fig.add_vline(x=date_str, line_dash="dot", line_color="rgba(43, 138, 62, 0.3)")
    except IndexError: pass 

fig.update_layout(title=f"【{asin_name}】 动态资金统筹与库存推演沙盘", xaxis_title="真实日历 (Date)", yaxis_title="FBA 可用库存 (Units)", hovermode="x unified", height=600, margin=dict(t=50, b=50))
